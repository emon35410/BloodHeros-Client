...
const DashboardLayout = () => {
    ...
    if (authLoading || dbLoading || roleLoading || !userData) {
        return (
    ...
    return (
        <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-[#0F172A] text-slate-200' : 'bg-[#F8FAFC] text-slate-900'}`}>
            ...
            <aside className={`fixed top-0 left-0 z-50 h-screen w-72 transform transition-all duration-300 
                ...
                <div className="flex flex-col h-full">
                    ...
                    <nav className="flex-1 overflow-y-auto px-4 space-y-6">
                        ...
                        {/* Management Section - Admin & Volunteer */}
                    ...
                    <div className={`p-4 border-t ${darkMode ? 'border-slate-800 bg-slate-900/50' : 'border-gray-100 bg-gray-50/50'}`}>
                        <div className="flex items-center justify-between mb-4 px-2">
                             ...
                             <button onClick={() => setDarkMode(!darkMode)} className={`p-2 rounded-lg ${darkMode ? 'bg-slate-800 text-yellow-400' : 'bg-white shadow-sm border border-gray-200'}`}>
            ...
            <div className="lg:ml-72 min-h-screen flex flex-col">
                <header className={`backdrop-blur-md sticky top-0 z-30 border-b px-6 py-4 ${darkMode ? 'bg-slate-900/80 border-slate-800' : 'bg-white/80 border-gray-100'}`}>
                    <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-lg bg-gray-100 dark:bg-slate-800">
